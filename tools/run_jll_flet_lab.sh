...
if [[ -f "$CONFIG" ]]; then
  ...
  printf 'LAB config chybí; Flet otevře Setup Wizard.\n'
fi
...
cd "$ROOT"
printf 'Spouštím Flet desktop UI (cílová prezentační vrstva)...\n'
printf 'Referenční PySide6: ./tools/run_jll_lab.sh\n'
exec "$PYTHON" -m jll.flet_ui \
