...
def test_windows_version_metadata_matches_project_version() -> None:
    ...
    assert "UninstallDisplayName={#MyProductName} {#MyAppVersion}" in iss
    assert "AppVerName={#MyProductName} {#MyAppVersion}" in iss
...
def test_setup_categories_use_scrollable_listview() -> None:
    text = (ROOT / "src/jll/flet_ui/screens/setup.py").read_text(encoding="utf-8")
    assert "ft.ListView(" in text
    assert "self.body.scroll = None" in text
    assert "self._nav" in text
